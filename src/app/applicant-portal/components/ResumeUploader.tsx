...
export default function ResumeUploader({ onConfirm, onCancel }: ResumeUploaderProps) {
    ...
    if (status === 'REVIEW' && parsedData) {
        return (
            <div className="bg-white rounded-2xl border border-border shadow-card p-8 max-w-3xl mx-auto max-h-[90vh] flex flex-col">
                <div className="flex justify-between items-start mb-6 shrink-0">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                            Please review and correct any missing or incorrect information before continuing.
                        </p>
                    </div>
                </div>

                <div className="flex-1 overflow-y-auto pr-2 scrollbar-thin space-y-6 py-2">
                    <div className="grid grid-cols-2 gap-5">
                        ...
                        <FieldGroup label="Key Skills (Comma separated)">
                            <input
                                type="text"
                    ...
                    <FieldGroup label="Work Experience Summary">
                        <textarea
                            rows={4}
                            value={parsedData.experience}
                            onChange={e => setParsedData({ ...parsedData, experience: e.target.value })}
                            className="w-full px-4 py-2 text-sm border border-border rounded-lg focus:ring-2 focus:ring-primary-700/20 outline-none transition-all resize-none"
                        />
                    </FieldGroup>

                    <FieldGroup label="Education History">
                        <textarea
                            rows={3}
                ...
                <p className="text-center text-[10px] text-muted-foreground mt-4">
                    By continuing, you confirm that the information provided is accurate and will be saved to your profile.
                </p>
            </div>
        );
    }
